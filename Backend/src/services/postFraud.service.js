...
/**
 * Check a post entity for fraud indicators
 * Scans: content for keywords (spam, phishing, hate_speech) and links
 */
export const check = async (body) => {
  ...
  for (const category of keywordCategories) {
    ...
    if (scanResult.flagged) {
      keywordResult = {
        flagged: true,
        reason: `Suspicious ${category} content detected in post`,
  ...
  if (id && finalResult.isFlagged) {
    try {
      ...
      console.error(`Failed to update fraud status for post ${id}:`, dbError.message);
    }
  }
